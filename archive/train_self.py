...
from src.train_function import train_model, eval_model, test_model, train_ul_model, train_model_result
...
def run(args):
    ...
    # generate data index
    if args.train_eval:
        qd_index, rels_index = data_processor.generate_data_index("train_2")
    ...
    for epoch in range(1, 1 + args.epochs):
        ...
        # model evaluation
        if epoch > 4:
            dev_precision_nn, dev_precision_2, dev_precision_21, dev_ndcg, dev_map, dev_mrr_nn, dev_mrr_2, dev_sims = eval_model(args, model, data_processor, vocab_q, vocab_d, device, epoch, dev_qd_index, dev_rels_index)
        # scheduler.step()
        ...
        if epoch > 5 or args.epochs < 6:
            if dev_ndcg > best_ndcg:
                ...
                if args.train_eval:
                    train_sims = train_model_result(
                        args, model, data_processor, vocab_q, vocab_d, device, epoch, qd_index, rels_index)
    ...
    if train_flag:
        ...
        if args.train_eval:
            torch.save(train_sims, "fr_train_osl_real")
            flat_index = [item for sublist in rels_index for item in sublist]
            torch.save(flat_index, "fr_train_index")
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_file', dest='train_file', type=str,
                        default='/home/liu1769/scratch/data_eng__french15/train_15.pkl', help='path to training file')
    ...
    parser.add_argument('--dev_file', dest='dev_file', type=str,
                        default='/home/liu1769/scratch/data_eng__french15/dev_15.pkl', help='path to development file')
    parser.add_argument('--test_file', dest='test_file', type=str,
                        default='/home/liu1769/scratch/data_eng__french15/test_15.pkl', help='path to test file')
    ...
    # loss function
    parser.add_argument('--relax_cos', dest='relax_cos', action='store_true',
                        help='whether use the relaxed cosine similarity')
    ...
    parser.add_argument('--epsilon', dest='epsilon', type=float, default=0.05,
                        help='epsilon for relaxed cosine function')
